...
export class FollowersUtils {
  ...
  static async unFollowUser(user, profile, dispatch) {
    const response = await followerService.unFollowUser(
      user?._id,
      profile?._id
    );
    Utils.dispatchNotification(response.data.message, "success", dispatch);
  ...
  static removeBlockedUser(profile, data) {
    ...
    if (profile?._id === data.blockedBy) {
      profile.blocked = [
        ...Utils.removeUserFromList(profile.blocked, data.blockedUser),
      ];
    }

    if (profile?._id === data.blockedUser) {
      profile.blockedBy = [
        ...Utils.removeUserFromList(profile.blockedBy, data.blockedBy),
      ];
    }
    return profile;
  }
